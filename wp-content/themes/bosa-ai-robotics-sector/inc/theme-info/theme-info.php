<?php
/**
 * Bosa AI Robotics Sector - Theme Info Admin Menu
 * @package Bosa AI Robotics Sector Themes
 * @subpackage Admin
 */
if ( ! class_exists( 'Bosa_Ai_Robotics_Sector_Theme_Info' ) ) {
    class Bosa_Ai_Robotics_Sector_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'bosa-ai-robotics-sector' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Setup', 'bosa-ai-robotics-sector' );
        ...
        public function update_recommended_action_callback() {
            ...
            /*from js action*/
            $action_id = esc_attr( isset( $_GET[ 'id' ] ) ? sanitize_text_field( wp_unslash( $_GET[ 'id' ] ) ) : 0 );
            $todo = esc_attr( isset( $_GET[ 'todo' ] ) ? sanitize_text_field( wp_unslash( $_GET[ 'todo' ] ) ) : '' );
        ...
        public function kt_theme_info_screen() {
            ...
            if (
                ...
                /* Display tabs */
                if ( ! empty( $this->tabs ) ) {
                    $current_tab = isset( $_GET['tab'] ) ? sanitize_text_field( wp_unslash( $_GET['tab'] ) ) : 'getting_started';
                    ...
                    foreach ( $gs_steps as $gs_step ) {
                        ...
                        if( $is_gs ) {
                            if ( is_plugin_active('advanced-import/advanced-import.php') && is_plugin_active('keon-toolset/keon-toolset.php') && is_plugin_active('kirki/kirki.php') && is_plugin_active('elementor/elementor.php') && is_plugin_active('breadcrumb-navxt/breadcrumb-navxt.php') && is_plugin_active( 'contact-form-7/wp-contact-form-7.php' )  && is_plugin_active( 'elementskit-lite/elementskit-lite.php' ) && is_plugin_active( 'bosa-elementor-for-woocommerce/bosa-elementor-for-woocommerce.php' ) ) {
                                echo ('<h3>'.esc_html__('Plugins added successfully. You have completed Step 1.', 'bosa-ai-robotics-sector').'</h3>');
                            echo '</div></div>';
                            }else {
                                if ( ! empty( $title ) ) {
                                    echo '<h3>';
                                ...
                                if ( ! empty( $button_label ) ) {
                                    ...
                                    $button_class = '';
                                    if ( $is_button ) {
                                        $button_class = 'button button-primary button-hero bosa-ai-robotics-sector-install-plugins';
                                    }
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'bosa-ai-robotics-sector' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'bosa-ai-robotics-sector' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'bosa-ai-robotics-sector' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'bosa-ai-robotics-sector' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'bosa-ai-robotics-sector').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'bosa-ai-robotics-sector' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . esc_attr( $preview_link ) . '">' . esc_html__( 'Live Preview', 'bosa-ai-robotics-sector' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        */
                    ...
                    /*defaults values for demos array */
                        ...
                        foreach ( $demos as $demos_item ) {
                            ...
                            if ( ! empty( $link_title ) && ! empty( $link_url ) ) {
                                ...
                                echo '<a target="' . esc_attr( $button_new_tab ) . '" href="' . esc_url( $demos_item['link_url'] ) . '"class="' . esc_attr( $button_class ) . '">' .  esc_html( $demos_item['link_title'] ) . '</a>';
                                echo '</p>';
                            }
                            echo '</div></div><!-- .col -->';
                        }
                ...
                if( ! empty($rating) ){
                    ...
                    foreach ( $rating as $rating_item ) {
                            ...
                            if ( ! empty( $link_title ) && ! empty( $link_url ) ) {
                                ...
                                echo '<a target="' . esc_attr( $button_new_tab ) . '" href="' . esc_url( $rating_item['link_url'] ) . '"class="' . esc_attr( $button_class ) . '">' . esc_html( $rating_item['link_title'] ) . '</a>';
                                echo '</p>';
                            }
                            echo '</div></div><!-- .col -->';
                    }
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            wp_enqueue_style( 'bosa-ai-robotics-sector-info-css', get_template_directory_uri() . '/inc/theme-info/assets/css/theme-info.css' );

            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {

                wp_enqueue_script( 'bosa-ai-robotics-sector-info-js', get_template_directory_uri() . '/inc/theme-info/assets/js/theme-info.js', array( 'jquery' ) );
                ...
                $count = $this->count_recommended_actions();
                wp_localize_script( 'bosa-ai-robotics-sector-info-js', 'kt_theme_info_object', array(
...
$bosa_ai_robotics_sector_theme_name_config = esc_attr ( wp_get_theme()->get('Name') );
$bosa_ai_robotics_sector_theme_name_config_url = strtolower( str_replace( ' ', '-', $bosa_ai_robotics_sector_theme_name_config ) );
$bosa_ai_robotics_sector_demo_name_config_url = 'ai-robotics-sector';
if( is_child_theme() ){
    $bosa_ai_robotics_sector_demo_name_config_url = str_replace( 'bosa-ai-robotics-sector-', '', $bosa_ai_robotics_sector_theme_name_config_url );
}

$bosa_ai_robotics_sector_config = array(

    // Main welcome title
    /* translators: %s - Theme Name*/
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'bosa-ai-robotics-sector' ), $bosa_ai_robotics_sector_theme_name_config ),

    // Main welcome content
    /* translators: %s - Theme Name*/
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'bosa-ai-robotics-sector' ), '<b>'.$bosa_ai_robotics_sector_theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Details', 'bosa-ai-robotics-sector' ),
            'url' => 'https://bosathemes.com/' .$bosa_ai_robotics_sector_theme_name_config_url 
        ),
        'demo_url'  => array(
            'text' => __( 'View Demo', 'bosa-ai-robotics-sector' ),
            'url' => 'https://demo.bosathemes.com/bosa/' .$bosa_ai_robotics_sector_demo_name_config_url
        ),
        'pro_url'  => array(
            'text' => __( 'View Pro Version', 'bosa-ai-robotics-sector' ),
            'url' => 'https://bosathemes.com/bosa-pro'
        ),
        'rate_url'  => array(
            'text' => __( 'Rate This Theme', 'bosa-ai-robotics-sector' ),
            'url' => 'https://wordpress.org/support/theme/' .$bosa_ai_robotics_sector_theme_name_config_url. '/reviews'
        ),  
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'bosa-ai-robotics-sector' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'bosa-ai-robotics-sector' ),
        'support'              => esc_html__( 'Support', 'bosa-ai-robotics-sector' ),
        'changelog'            => esc_html__( 'Changelog', 'bosa-ai-robotics-sector' ),
        'free_pro'             => esc_html__( 'Free VS Pro', 'bosa-ai-robotics-sector' ),
        'demos'                => esc_html__( 'Demos', 'bosa-ai-robotics-sector' ),
        'rating'               => esc_html__( 'Rate Theme', 'bosa-ai-robotics-sector' )
    ),

    /*Getting started tab*/
    'gs_steps' => array(
        'first' => array(
            'title' => esc_html__( 'Step 1 - To fully take advantage of the best our theme can offer, get started.', 'bosa-ai-robotics-sector' ),
            /* translators: %s - Theme Name*/
            'button_label' => sprintf(esc_html__( 'Get started with %s', 'bosa-ai-robotics-sector' ), $bosa_ai_robotics_sector_theme_name_config ),
            'is_button' => true,
        ...
        'second' => array(
            'title' => esc_html__( 'Step 2 - Checkout starter sites (Demos)', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'All starter demos sites comes attached to Bosa AI Robotics Sector just ready to pluck one from. Discover them, be enamoured and finally decide on the one you resonate with. If one doesnt catch your fancy dont be afraid to have a go at building a site with your own design.', 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'View Demo', 'bosa-ai-robotics-sector' ),
            'button_link' => esc_url( 'https://demo.bosathemes.com/bosa/' .$bosa_ai_robotics_sector_demo_name_config_url ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'third' => array (
            'title' => esc_html__( 'Step 3 - Import demo of your choice', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'Once you have a found a site you like, you can move ahead and import it. It is to be noted that some necessary plugins will be installed to function the site as you see in the demo site. A fresh Wordpress site is recommended to mirror the demo site on your setup. Importing the demo on an already running site will create discrepancies with the site shown on the demo.', 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'Import Demo', 'bosa-ai-robotics-sector' ),
            'button_link' => esc_url( admin_url( 'themes.php?page=advanced-import' ), 'bosa-ai-robotics-sector' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array(
            'title' => esc_html__( 'Step 4 - Start editing content and making your site!', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'Once finishing the import you play with the demo to your hearts content.', 'bosa-ai-robotics-sector' ),
        ),
        'fifth' => array (
            'title' => esc_html__( "Step 5 - That's it!", 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'Go live with the website and enjoy it!', 'bosa-ai-robotics-sector' ),
        ),
    ),
    ...
    'support_content' => array(
        'first' => array(
            'title' => esc_html__( 'Need more features?', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.', 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'Upgrade to PRO', 'bosa-ai-robotics-sector' ),
            'button_link' => esc_url( 'https://bosathemes.com/bosa-pro' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array (
            'title' => esc_html__( 'Go to Customizer', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'All Settings, Header & Footer Options, Theme Options, Widgets and Menus are available via Customize screen.', 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'Go to Customizer', 'bosa-ai-robotics-sector' ),
            'button_link' => esc_url( admin_url( 'customize.php' ) ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'third' => array(
            'title' => esc_html__( 'Documentation', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use Bosa AI Robotics Sector.', 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'Read full documentation', 'bosa-ai-robotics-sector' ),
            'button_link' => esc_url( 'https://bosathemes.com/docs/bosa' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array (
            'title' => esc_html__( 'Support Forum', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'Got theme support question or found bug? Simply create a support ticket with your queries and we will get back to you.', 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'Support Forum', 'bosa-ai-robotics-sector' ),
            'button_link' => esc_url( 'https://bosathemes.com/user-login-register' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fifth' => array(
            'title' => esc_html__( 'Got sales related question?', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.", 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'Pre-sale Question?', 'bosa-ai-robotics-sector' ),
            'button_link' => esc_url( 'https://bosathemes.com/pre-sale' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'sixth' => array(
            'title' => esc_html__( 'Customization Request', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'Needed any customization for the theme, you can request from here.', 'bosa-ai-robotics-sector' ),
            'button_label' => esc_html__( 'Customization Request', 'bosa-ai-robotics-sector' ),
    ...
    'free_pro' => array (
        'first'=> array(
            'title' => esc_html__( 'Free VS Pro Features', 'bosa-ai-robotics-sector' ),
            /* translators: %s - Theme Name*/
            'desc' => sprintf( esc_html__( '%s is a multipurpose free WordPress theme. However, Bosa Pro comes with more cool and awesome features that give you a flexible and wide range of options to build a full-fledged website.', 'bosa-ai-robotics-sector' ), $bosa_ai_robotics_sector_theme_name_config ),
            'link_title' => esc_html__( 'Check All Features', 'bosa-ai-robotics-sector' ),
    ...
    'demos' => array (
        'first'=> array(
            'title' => esc_html__( 'Pre-built Demos', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__ ( 'To get started with ready-made starter site templates.', 'bosa-ai-robotics-sector' ),
            'link_title' => esc_html__( 'View All Demos', 'bosa-ai-robotics-sector' ),
            'link_url' => esc_url( 'https://demo.bosathemes.com/bosa/' .$bosa_ai_robotics_sector_demo_name_config_url ),
            'is_button' => true,
            'recommended_actions' => false,
            'is_new_tab' => true
        ),
    ...
    'rating' => array(
        'first'=> array(
            'title' => esc_html__( 'Show us how much you like our theme.', 'bosa-ai-robotics-sector' ),
            'desc' => esc_html__( 'If you like our work, please give us a moment of your time to rate our theme 5 stars on ', 'bosa-ai-robotics-sector' ) .'<a target="_blank" href="https://wordpress.org/support/theme/' .$bosa_ai_robotics_sector_theme_name_config_url. '/reviews">' . 'wordpress.org' . '</a>' . esc_html__('. It will give us more energy to work on this theme.', 'bosa-ai-robotics-sector' ),
            'link_title' => esc_html__( 'Rate Theme', 'bosa-ai-robotics-sector' ),
            'link_url' => esc_url( 'https://wordpress.org/support/theme/' .$bosa_ai_robotics_sector_theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        )
    )
);
return new Bosa_Ai_Robotics_Sector_Theme_Info( $bosa_ai_robotics_sector_config );